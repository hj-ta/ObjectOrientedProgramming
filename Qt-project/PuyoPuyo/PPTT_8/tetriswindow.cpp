...
{
    int ROW = p_game->rowsCount; // 게임 보드의 행 수
    int COL = p_game->colsCount; // 게임 보드의 열 수
    ...
    std::queue<int> tempQueue = p_game->nextQueue; // 복사본 사용
...
void TetrisWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Right:
        ...
        break;
    case Qt::Key_Z:
        p_game->rotateShapeCounterClockwise();
        drawNext();
        break;
    case Qt::Key_X:
        p_game->rotateShapeClockwise();
        drawNext();
        break;
    case Qt::Key_Space:
        p_game->dropBlock();
    ...
    for (int i = 0; i < shape.size(); ++i)
    {
        for (int j = 0; j < shape[i].size(); ++j)
        {
            if (shape[i][j] == 1)
            {
                ...
                if (row >= p_game->rowsCount || p_game->board[row][col] != "Empty")
                {
                    return false;
                }
            }
    ...
    for (int i = 0; i < shape.size(); ++i)
    {
        for (int j = 0; j < shape[i].size(); ++j)
        {
            if (shape[i][j] == 1)
            {
                int row = p_game->axis_row + i;
                int col = next_col + j;
                if (col >= p_game->colsCount || p_game->board[row][col] != "Empty")
                {
                    return false;
                }
            }
...
void TetrisWindow::drawNext()
{
    if (p_game->gameover)
    {
        for (int i = 0; i < p_game->rowsCount; i++)
            for (int j = 0; j < p_game->colsCount; j++)
            {
